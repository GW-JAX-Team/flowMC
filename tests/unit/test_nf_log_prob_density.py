...
def _models():
    ...
    yield "realNVP", RealNVP(N_DIM, 4, 16, key, data_mean=DATA_MEAN, data_cov=DATA_COV)
...
def test_log_prob_integrates_to_one(name):
    ...
    assert ess > 2000, f"proposal does not cover the flow (ESS={ess:.0f}); test is void"
    ...
    assert abs(Z - 1.0) < 0.05, (
        f"{name}: exp(log_prob) integrates to {Z:.5f}, not 1 "
        f"(prod(sqrt(diag(data_cov))) = "
        f"{float(jnp.prod(jnp.sqrt(jnp.diag(DATA_COV)))):.5f} "
        "-- missing standardization Jacobian?)"
    )
...
def test_log_prob_batched_matches_looped(name):
    ...
    assert batched.shape == (64,), f"batched log_prob returned {batched.shape!r}"
    looped = np.array([float(model.log_prob(xi)) for xi in x])
    assert np.allclose(batched, looped, rtol=1e-6, atol=1e-5), (
        f"{name}: batched and looped log_prob differ by up to "
        f"{np.max(np.abs(batched - looped)):.3g}"
    )
...
def test_identity_data_cov_is_unaffected():
    ...
    m = MaskedCouplingRQSpline(
        N_DIM, 3, [16, 16], 4, key, data_mean=jnp.zeros(N_DIM), data_cov=jnp.eye(N_DIM)
    )
